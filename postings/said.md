...
4. The Blake3-256 digest is then computed on that serialized **json**: **said**:
...
- Module **SAD** provides trait **SAD** that has functions:
  ...
  - **compute digest** : computes the **said** of a data structure, places it in a chosen field.
...
- Fields attributes: **said** marks field that should be replaced by computed digest during **compute digest**.
...
- [SAD crate](https://github.com/THCLab/cesrox/tree/master/said/src/sad) **use said::sad::SAD** defines the **trait** with two functions **compute digest** and **derivation_data** which are implemented using **Procedural Macros**.
...
> why **SAD** macro?
> Starting with the **SAD** trait which implements two functions: **compute digest** and **derivation_data**. A **SAD** macro is created to avoid structs to individually implementing these functions but rather depend on the **SAD** macro abstraction. For usability the **SAD** macro the structs have to be annotated with **#[derive](SAD)**, and implement **Serialize** trait.
...
This function hides all the manipulations of placing the computed digest inside a struct when annotated with **i#[derive(SAD)]\*, thus the name **said\*\*.
...
The full code of the **imp_compute_digest** can be found [here](https://github.com/THCLab/cesrox/blob/master/said/sad_macros/src/lib.rs). The below is the dissection of the code to help understand how each part play a role in manipulating the structs annotated with \*_i#[derive(SAD)]_.
